...
class SkipConnectionModel(nn.Module):
    ...
    def __init__(self, f_in, f_out, *args):        
        ...
        self.ln = LayerNorm(args[4])
        self.ln1 = LayerNorm(args[3])
        self.ln2 = LayerNorm(args[2])
        self.ln3 = LayerNorm(args[1])
        
        self.upblock1 = nn.Sequential(nn.Linear(f_in, args[1]),GELU(),nn.BatchNorm1d(args[1]))
        self.upblock2 = nn.Sequential(nn.Linear(args[1],args[2]),GELU(),nn.BatchNorm1d(args[2]))
        self.upblock3 = nn.Sequential(nn.Linear(args[2],args[3]), GELU(),nn.BatchNorm1d(args[3]))
        self.upblock4 = nn.Sequential(nn.Linear(args[3],args[4]),GELU(),nn.BatchNorm1d(args[4]))

        self.downblock1 = nn.Sequential(nn.Linear(args[4], args[3]),GELU(),nn.BatchNorm1d(args[3]))
        self.downblock2 = nn.Sequential(nn.Linear(args[3], args[2]),GELU(),nn.BatchNorm1d(args[2]))
        self.downblock3 = nn.Sequential(nn.Linear(args[2], args[1]),GELU(),nn.BatchNorm1d(args[1]))
        self.downblock4 = nn.Sequential(nn.Linear(args[1], args[0]),GELU(),nn.BatchNorm1d(args[4]))
        
        self.fclayer = nn.Sequential(nn.Linear(args[0], f_out))
